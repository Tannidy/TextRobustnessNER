...
def _get_entailed_hypothesis(tokens, index, number):
    ...
    Args:
        tokens: the text
        index: the position for the number word
        number: number word (int)

    Returns: The sentence that be changed the number word
...
def _get_contradictory_hypothesis(tokens, index, number):
    """
     ...
     return a sentence that have the contradiction semantic with the original sample.
