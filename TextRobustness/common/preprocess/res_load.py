...
from TextRobustness.common.utils import url_downloader, module_loader, pickle_loader
...
def model_load():
    ...
    for module in module_loader(dir_path, 'nltk'):
        if hasattr(module, "NAME") and hasattr(module, "DOWNLOAD"):  # is a data module
            tmp = {"name": module.NAME}

            if callable(module.DOWNLOAD):
                tmp["download"] = module.DOWNLOAD
            elif isinstance(module.DOWNLOAD, str):
                tmp["download"] = url_downloader(module.DOWNLOAD)
            else:
                raise ValueError(
                    "Data Module: %s\n dir: %s\n type: %s"
                    % (module, dir(module), type(module.DOWNLOAD))
                )

            if hasattr(module, "LOAD"):
                tmp["load"] = module.LOAD
            else:
...
model_list = model_load()


class ModelManager(object):
    """
    ModelManager is a module that manages all the resources used in Attacker, Metric, Substitute, TextProcessors and utils.

    You can use 

    .. code-block:: python
    
        for data_name in ModelManager.AVAILABLE_DATAS:
            ModelManager.download(data_name)
     
    to download all the available resources, but this is not recommend because of the huge network cost.

    ``ModelManager.load`` and ``ModelManager.download`` is a alias of
    ``TextRobustness.ModelManager.load`` and ``TextRobustness.ModelManager.download``, they are exactly equivalent.
    These two methods are useful for both developer and user, that's the reason we provide shortter name for them.
    """

    AVAILABLE_DATAS = [x["name"] for x in model_list]

    data_path = {
        x["name"]: os.path.join(DATA_PATH, x["name"]) for x in model_list
    }

    data_download = {x["name"]: x["download"] for x in model_list}

    data_loader = {x["name"]: x["load"] for x in model_list}
    ...
    def load(cls, data_name, cached=True):
        ...
        if not os.path.exists(cls.data_path[data_name]):
            if cls.__auto_download:
                ...
                raise ValueError(data_name, cls.data_path[data_name])
