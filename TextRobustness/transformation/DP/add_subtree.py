...
from TextRobustness.common.settings import (WIKIDATA_STATEMENTS, CLAUSE_HEAD,
                                            WIKIDATA_INSTANCE)
...
class AddSubtree(Transformation):
            ...
            if i >= n:
                ...
                if clause_add:
                    ...
                    for token in tokens_add[::-1]:
                        sample_mod = sample.insert_field_before_index('x', word_id[-1], token)
    ...
    def search_list(self, query):
        ...
        Args:
            query: str
                A list of words in the entity, which is joined by '%20'.
    ...
    def clause_generate(self, entity_id):
        ...
        Args:
            entity_id: str
                The ID of the given Wikidata entry.
...
if __name__ == '__main__':
    ...
    ads = AddSubtree()
    samples = ads.transform(sample)
    for sample in samples:
        print(sample.dump())
    T2 = time.perf_counter()
    print(f'{T2 - T1} seconds used.')
